...
def read_files(path):
    ...
    for p in range(pack_number):
        header_name_to_num, rows = read_files_pack(
            files[int(f_number / pack_number * p): int(f_number / pack_number * (p + 1))])
        all_data = array(rows)
        del rows
        ...
        print(f'{(p + 1) / pack_number * 100}% processed', all_data_x.shape, all_data_y.shape, data_x.shape,
              data_y.shape)
