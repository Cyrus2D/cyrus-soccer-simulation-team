//
// Created by nader on 2022-07-08.
//
...
bool go_to_target(PlayerAgent * agent, Vector2D target){
    ...
    double angle_thr = 20;
    if(!Body_GoToPoint(target, 1, dash_power,1.2,3,false,30).execute(agent)){
        if(wm.self().body().abs() < 15 && wm.existKickableTeammate()){
            agent->doDash(100);
        }else{
...
bool IntentionScape::finished(  rcsc::PlayerAgent * agent ){
    ...
    if(wm.existKickableOpponent())
        return true;
    double offside_line_x = bhv_scape_voronoi::maxXLow(wm);
    if (wm.existKickableTeammate())
        offside_line_x = bhv_scape_voronoi::maxXHigh(wm);
    if(M_target_point.x > offside_line_x)
        return true;
...
bool IntentionScape::execute( rcsc::PlayerAgent * agent ){
    ...
    Vector2D target = M_target_point;
    if (wm.existKickableTeammate())
        target.x = std::min(bhv_scape_voronoi::maxXHigh(wm), target.x);
    else
        target.x = std::min(bhv_scape_voronoi::maxXLow(wm), target.x);

    go_to_target(agent, target);
...
bool bhv_scape_voronoi::execute(PlayerAgent *agent) {
    ...
    if (best.isValid()) {
        ...
        if (wm.existKickableTeammate())
            best.x = std::min(maxXHigh(wm), best.x);
        else
            best.x = std::min(maxXLow(wm), best.x);
...
double bhv_scape_voronoi::maxXLow(const WorldModel & wm){
...
vector<Vector2D> bhv_scape_voronoi::voronoi_points(rcsc::PlayerAgent *agent) {
    ...
    double offside_line_x = maxXHigh(wm);
...
//            if(is_near_me && bhv_scape_voronoi::can_receive_th_pass(wm, v_points[i])){
        if(is_near_me && v_points[i].absY() < 33.5){
