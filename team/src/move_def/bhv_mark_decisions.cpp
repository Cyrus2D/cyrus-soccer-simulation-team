...
void BhvMarkDecisionGreedy::midMarkThMarkCostFinder(const WorldModel &wm, double mark_eval[][12], bool used_hpos,
    ...
    for (size_t t = 2; t <= 11; t++) {
        ...
        for (size_t o = 1; o <= 11; o++) {
            ...
            if (opp->unum() == fastest_opp && !fastest_opp_marked) {
                ...
                dist_tm_opp = (opp_pos.pos.dist(tm_pos) * pos_z + opp_pos.pos.dist(tm_hpos) * hpos_z) / (hpos_z + pos_z);
    ...
    for (size_t o = 1; o <= 11; o++) {
        for (size_t t = 1; t <= 11; t++) {
            dlog.addText(Logger::MARK, "opp %d tm %d = %.1f , pos = %.1f , hpos = %.1f", o, t, mark_eval[o][t], mark_eval_pos[o][t], mark_eval_hpos[o][t]);
        }
    }
    #endif
}
...
void BhvMarkDecisionGreedy::midMarkLeadMarkOffensiveCostFinder(const WorldModel &wm, double mark_eval[][12],
                                                                     bool used_hpos, vector<double> block_eval,
    ...
    for (int t = 2; t <= 11; t++) {
        ...
        for (int o = 1; o <= 11; o++) {
            ...
            if (tm_pos.dist(opp_pos.pos) < 5) {
                max_hpos_dist = Setting::i()->mDefenseMove->mMidProj_HPosMaxDistMark * 1.5;
            }

            if (opp->unum() == fastest_opp) {
                max_hpos_dist = Setting::i()->mDefenseMove->mMidProj_HPosMaxDistBlock;
                max_pos_dist = Setting::i()->mDefenseMove->mMidProj_PosMaxDistBlock;
                opp_pos.pos = ball_inertia;
                if (tm_pos.dist(opp_pos.pos) < 10 && opp_pos.pos.x < tm_hpos_def_line + 15) {
                    max_hpos_dist = Setting::i()->mDefenseMove->mMidProj_HPosMaxDistBlock * 0.9;
                }
                if (Strategy::i().tm_Post(t) == Strategy::i().player_post::pp_ch){
                    max_hpos_dist = Setting::i()->mDefenseMove->mMidProj_HPosMaxDistBlock * 0.6;
                    const AbstractPlayerObject * tm5 = wm.ourPlayer(5);
                    const AbstractPlayerObject * tm2 = wm.ourPlayer(2);
                    if(!tm2 || !tm5 || tm2->unum() != 2 || tm5->unum() != 5 || tm2->pos().dist(tm5->pos()) > 15){
                        max_hpos_dist = Setting::i()->mDefenseMove->mMidProj_HPosMaxDistBlock * 0.4;
                    }
                }
                if (tm_pos.dist(opp_pos.pos) > max_pos_dist) {
                    #ifdef DEBUG_PRINT
        ...
        for (int o = 1; o <= 11; o++) {
            ...
            if (opp->unum() == fastest_opp) {
                ...
                if (Strategy::i().tm_Post(t) == Strategy::i().player_post::pp_ch
                    ...
                    const AbstractPlayerObject * tm2 = wm.ourPlayer(2);
                    if(!tm2 || !tm5 || tm2->unum() != 2 || tm5->unum() != 5 || tm2->pos().dist(tm5->pos()) > 15){
                        max_hpos_dist = Setting::i()->mDefenseMove->mMidNear_HPosMaxDistBlock * 0.5;
                    }
                }
                if (tm_pos.dist(opp_pos.pos) > max_pos_dist) {
                    #ifdef DEBUG_PRINT
...
bool BhvMarkDecisionGreedy::canCenterHalfMarkLeadNear(const WorldModel &wm, int t, Vector2D opp_pos, Vector2D ball_inertia){
    if (Strategy::i().tm_Post(t) != Strategy::i().player_post::pp_ch)
        return true;
    const AbstractPlayerObject * tm5 = wm.ourPlayer(5);
    const AbstractPlayerObject * tm2 = wm.ourPlayer(2);
    ...
    if(opp_pos.y > avg_y + 15 || opp_pos.y < avg_y - 15)
        return false;
    if(opp_pos.x > (tm2->pos().x + tm5->pos().x) / 2 + 10)
        return false;
    return true;
}
    ...
    for (int t = 2; t <= 11; t++) {
        ...
        for (int o = 1; o <= 11; o++) {
            ...
            if (opp->unum() == fastest_opp) {
                ...
                if ((top_corner_rect_ball.contains(ball_pos) && top_corner_rect.contains(opp->pos()) && top_corner_rect.contains(tm_hpos))
                    || (bottom_corner_rect_ball.contains(ball_pos) && bottom_corner_rect.contains(opp->pos()) && bottom_corner_rect.contains(tm_hpos))){
...
void BhvMarkDecisionGreedy::midMarkDecision(PlayerAgent *agent, MarkType &mark_type, int &mark_unum, bool &blocked) {
    ...
    //Tm Def Mark Opp Off
    {
        ...
        for(int i = 0; i < temp_opps.size(); i++){
            int o1 = temp_opps[i];
            if (wm.theirPlayer(o1)->pos().x < Strategy::i().getPosition(2).x + Setting::i()->mDefenseMove->mMidTh_XNearOpps){
            ...
            for(int j = 0; j < temp_opps.size(); j++){
                ...
                if(o1_target.dist(o2_target) < Setting::i()->mDefenseMove->mMidTh_DistanceNearOpps){
                    dlog.addText(Logger::MARK, "----opp %d is near", o2);
    ...
    if (use_proj_mark) {
        ...
        for (size_t t = 2; t <= 11; t++) {
            ...
            if (Strategy::i().tm_Line(t) == Strategy::PostLine::forward)
                continue;
            #ifdef DEBUG_PRINT
            dlog.addText(Logger::MARK, "Tm %d can LP Mark", t);
            #endif
            temp_tms.push_back(t);
        ...
        for (size_t d = 1; d <= 4; d++) {
            ...
            size_t o = opp_eval[d].first;
            const AbstractPlayerObject *Opp = wm.theirPlayer(o);
            if (Opp->pos().x > wm.ourDefenseLineX() + 15)
                continue;
            if (opp_marker[o] != 0 && Opp->distFromBall() > 20)
                continue;
            #ifdef DEBUG_PRINT
            dlog.addText(Logger::MARK, "Opp %d can LP Marked", o);
            #endif
            temp_opps.push_back(o);
        }
        if (std::find(temp_opps.begin(), temp_opps.end(), fastest_opp) == temp_opps.end()) {
            if (fastest_opp > 0)
                temp_opps.push_back(fastest_opp);
        }
