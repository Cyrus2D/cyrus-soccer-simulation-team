...
#ifndef CYRUS_DataExtractor2_H
#define CYRUS_DataExtractor2_H
...
private:
    ...
    public:
        ...
        DataSide2 card;
        DataSide2 pos;
        DataSide2 relativePos;
        DataSide2 polarPos;
        DataSide2 vel;
        DataSide2 polarVel;
        DataSide2 counts;
        DataSide2 isKicker;
        DataSide2 openAnglePass;
        DataSide2 nearestOppDist;
        DataSide2 polarGoalCenter;
        DataSide2 openAngleGoal;
        DataSide2 in_offside;

        DataSide2 dribleAngle;
        int nDribleAngle;

        WorldMode2 input_worldMode;
        WorldMode2 output_worldMode;


        PlayerSortMode2 playerSortMode;
        bool kicker_first;
        bool use_convertor;
        int history_size;
        Option();
...
//                bool update_shoot=false);
    ...
    void get_data(const DEState & wm);


    //accessors
    static DataExtractor2 &i();
    static bool active;
...
private:
    ...
    void extract_pos(const rcsc::AbstractPlayerObject *player, const rcsc::WorldModel &wm, DataSide2 side);

    void extract_vel(const rcsc::AbstractPlayerObject *player, DataSide2 side);

    void extract_pass_angle(const rcsc::AbstractPlayerObject *player, const rcsc::WorldModel &wm, DataSide2 side);

    void extract_goal_polar(const rcsc::AbstractPlayerObject *player, DataSide2 side);

    void extract_goal_open_angle(const rcsc::AbstractPlayerObject *player, const rcsc::WorldModel &wm, DataSide2 side);

    void extract_base_data(const rcsc::AbstractPlayerObject *player, DataSide2 side);

    void extract_type(const rcsc::AbstractPlayerObject *player, DataSide2 side);

    void extract_history(const rcsc::AbstractPlayerObject *player, DataSide2 side);
...
public:
    ...
    Polar2(rcsc::Vector2D p);
};


#endif //CYRUS_DataExtractor2_H
