...
//    	return true;
//    }
	if(goalie_dist_next_ball < wm.getOpponentGoalie()->playerTypePtr()->kickableArea()+0.3)
	{
...
//	    }
	    ...
	    if(wm.getOpponentGoalie())
	    {
	    	if(wm.ball().pos().x > 0.0)
	    	   y=(wm.getOpponentGoalie()->body().degree()> 30)?-30:30;
	    	if(wm.ball().pos().x < 0.0)
	    	   y=(wm.getOpponentGoalie()->body().degree()< -30)?30:-30;
	    }
...
//			)
	...
	//    double diftime = wm.time().cycle() - wm.penaltyKickState()->time().cycle();
	    ...
	    if(wm.getOpponentGoalie())
	    {
	    	if(wm.self().pos().dist(wm.getOpponentGoalie()->pos())> 22)
	    	{
	    		kick_power = 1.14;
	    	}
	    	if(wm.self().pos().dist(wm.getOpponentGoalie()->pos())< 10)
	    	{
	    		if(kick_power > 0.65)kick_power=0.65;
	    	}
	    	if(wm.self().pos().dist(wm.getOpponentGoalie()->pos())< 7)
	    	{
...
//      		}
      	...
      	if(ball.absY() < 10)
      	{
      		if(target_angle.abs()>130
      				|| target_angle.abs()<50)target_angle= ( ball.y > me.y  ? 90.0 : -90.0 );
      	}
 		...
 		double y=l.getY(sign(wm.self().pos().x)*52.5);
  		if(fabs(y)<15)
  			target_angle= (  ball.y > me.y ? 90.0 : -90.0 );
  		if(me.x< 0 && me.y <-15)
...
////////////////////////////////////////////////////////////////
      	  ...
      	  if(!Body_GoToPoint( move_pos,0.4,SP.maxDashPower(),-1,100,false,20).execute( agent ))
      	  {
      		agent->setNeckAction( new rcsc::Neck_TurnToBall() );
#if log
