			...
			++o )
	{
		...
		else if ( opp_dist < param.tackleDist() - 0.2 )
		{
			...
			if(body_angle_dif < 90.0){
				...
				if(opp_next.dist(ball_on_tackle_line) < param.tackleDist() - 0.1){
#ifdef DEBUG
					dlog.addText( Logger::ACTION,
...
Body_HoldBall2008::keepReverse( PlayerAgent * agent )
{
	...
	double keep_dist = min_dist;
	double max_dist
	= wm.self().playerType().playerSize()
	+ wm.self().playerType().kickableMargin() * 0.7
	+ ServerParam::i().ballSize();

	for ( ; keep_dist < max_dist; keep_dist += 0.05 )
	{
		Vector2D keep_pos
		= my_inertia
		...
		Vector2D ball_move = (keep_pos - wm.ball().pos())/2.0;
		if((ball_move + wm.ball().pos()).dist(wm.self().inertiaPoint(1)) < min_dist)
			continue;
		Vector2D kick_accel = ball_move - wm.ball().vel();
		double kick_power = kick_accel.r() / wm.self().kickRate();
