 ...
 */
bool
Body_GoalieGoToPoint::execute( PlayerAgent * agent )
{
#ifdef DEBUG_PRINT
 ...
 */
void
Body_GoalieGoToPoint::checkGoalPost( const PlayerAgent * agent )
{
 ...
 */
bool
Body_GoalieGoToPoint::doAdjustDash( PlayerAgent * agent )
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();
 ...
 */
bool
Body_GoalieGoToPoint::doTurn( PlayerAgent * agent )
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();
    ...
    if(target_rel.th().abs() > 100) {
        ...
        {
            if(wm.ball().inertiaPoint(wm.interceptTable()->opponentReachCycle()).x < -10 || wm.interceptTable()->opponentReachCycle() < wm.interceptTable()->teammateReachCycle()){
                M_back_mode = true;
                turn_moment += 180.0;
#ifdef DEBUG_PRINT
            dlog.addText( Logger::ACTION,
                          __FILE__": (doTurn) back mode. (Avoid turning back) turn_moment=%.1f",
                          turn_moment.degree() );
#endif
            }
 ...
 */
bool
Body_GoalieGoToPoint::doDash( PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
