...
Bhv_GoToPointLookBall::execute( PlayerAgent * agent )
{
    ...
    int min_cycle = std::min(std::min(self_min, mate_min), opp_min);

    const Vector2D my_next = wm.self().pos() + wm.self().vel();
    Vector2D ball_next = wm.ball().pos() + wm.ball().vel();

    if(wm.ball().velCount() < 4){
        ball_next = wm.ball().inertiaPoint(min_cycle);
    }
    ...
    if ( ball_target_angle_diff.abs()
         < ServerParam::i().maxNeckAngle() - 5.0 + wm.self().viewWidth() / 2.0)
    {
