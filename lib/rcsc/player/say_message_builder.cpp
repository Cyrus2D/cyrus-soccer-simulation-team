...
MarkMessage::toStr( std::string & to ) const
{
    ...
    AudioCodec::i().encodeIntToStr(M_unum_list,M_mark_list,msg);
    dlog.addText( Logger::SENSOR,
                  "MarkMessage. success!"
                  " p1->%d %d,p2->%d %d,p3->%d %d,p4->%d %d,p5->%d %d,p6->%d %d,p7->%d %d,p8->%d %d in %s",
                  M_unum_list[0],M_mark_list[0],M_unum_list[1],M_mark_list[1],M_unum_list[2],M_mark_list[2],M_unum_list[3],M_mark_list[3],M_unum_list[4],M_mark_list[4],M_unum_list[5],M_mark_list[5],M_unum_list[6],M_mark_list[6],M_unum_list[7],M_mark_list[7],
                  msg.c_str() );
    to += header();
    to += msg;
...
MarkMessage::printDebug( std::ostream & os ) const
{
    ...
    os << "[Mark],"<<"1->"<<M_unum_list[0]<<" 2->"<<M_unum_list[1]<<" 3->"<<M_unum_list[2]<<" 4->"<<M_unum_list[3]<<" 5->"<<M_unum_list[4]<<" 6->"<<M_unum_list[5]<<" 7->"<<M_unum_list[6]<<" 8->"<<M_unum_list[7];
    return os;
}
...
*/
bool
PrePassMessage::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
bool
PreCrossMessage::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
bool
OnePlayerMessage1::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
bool
OnePlayerMessage2::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
    ...
    dlog.addText( Logger::SENSOR,
                  "OnePlayerMessage2. success!. unum = %d pos=(%.1f %.1f) -> [%s]",
                  M_unum,
                  M_player_pos.x, M_player_pos.y,
                  msg.c_str() );
...
*/
bool
TwoPlayerMessage01::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
    ...
    if ( dlog.isEnabled( Logger::SENSOR ) )
    {
        ...
        {
            dlog.addText( Logger::SENSOR,
                          "TwoPlayerMessage01. success!. unum=%d pos=(%.1f %.1f)",
                          M_player_unum[i],
                          M_player_pos[i].x, M_player_pos[i].y );
        }
...
*/
bool
TwoPlayerMessage02::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
    ...
    if ( dlog.isEnabled( Logger::SENSOR ) )
    {
        ...
        {
            dlog.addText( Logger::SENSOR,
                          "TwoPlayerMessage02. success!. unum=%d pos=(%.1f %.1f)",
                          M_player_unum[i],
                          M_player_pos[i].x, M_player_pos[i].y );
        }
...
*/
bool
ThreePlayerMessage001::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
*/
bool
ThreePlayerMessage002::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
*/
bool
ThreePlayerMessage011::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
*/
bool
ThreePlayerMessage012::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
...
*/
bool
ThreePlayerMessage022::toStr( std::string & to ) const
{
    if ( (int)to.length() + slength() > ServerParam::i().playerSayMsgSize() )
    {
        dlog.addText( Logger::SENSOR,
