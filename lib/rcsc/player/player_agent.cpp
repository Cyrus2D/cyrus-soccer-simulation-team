...
namespace rcsc {
    MSecTimer PlayerAgent::M_cycle_time_start = MSecTimer();
    int PlayerAgent::M_cycle_max_time = 80;
///////////////////////////////////////////////////////////////////////
/*!
...
PlayerAgent::initImpl( CmdLineParser & cmd_parser )
{
    ...
    PlayerObject::set_count_thr_opp_goalie(  config().playerOppGoaliePosCountThr(),
                                             config().playerVelCountThr(),
                                             config().playerFaceCountThr() );
...
{
    PlayerAgent::M_cycle_time_start.restart();
    PlayerAgent::M_cycle_max_time = this->config().cycleMaxTime();
...
PlayerAgent::parse( const char * msg )
{
    ...
    dlog.addText(Logger::WORLD,"msg: %s",msg);
...
PlayerAgent::Impl::analyzeSenseBody( const char * msg )
{
    ...
    // analyze process
    dlog.addText( Logger::SENSOR,
                  "===receive sense_body:%s", std::string(msg).c_str() );
...
PlayerAgent::Impl::analyzeHear( const char * msg )
{
    ...
    if ( ! analyzeCycle( msg, false ) )
    {
    	std::cout<<"###analyzeCycle error"<<std::endl;
        return;
    }
    // parse sender info
    long cycle;
...
PlayerAgent::doChangeView( const ViewWidth & width )
{
    if ( M_impl->see_state_.isSynch() )
    {
        dlog.addText(Logger::SENSOR, "IS SYNCH SEE");
    ...
    {
        dlog.addText(Logger::SENSOR, "IS NOT SYNCH SEE");
        if ( world().gameMode().type() != GameMode::PlayOn )
        {
            dlog.addText( Logger::ACTION,
...
double
PlayerAgent::cycleTimeUtilNow(){
    return PlayerAgent::M_cycle_time_start.elapsedReal();
}

void
PlayerAgent::setCycleMaxTime(int max_time){
    PlayerAgent::M_cycle_max_time = max_time;
}

bool PlayerAgent::canProcessMore(){
    if (PlayerAgent::cycleTimeUtilNow() > PlayerAgent::M_cycle_max_time){
        return false;
    }
    return true;
}
