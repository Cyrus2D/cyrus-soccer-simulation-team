...
*/
int
PlayerIntercept::predict( const WorldModel & wm,const PlayerObject & player,
...
*/
bool
PlayerIntercept::canReachAfterTurnDash( const WorldModel & wm,
                                        const int cycle,
                                        ...
                                        const Vector2D & ball_pos ) const
{
    ...
    Vector2D vel = player.vel();
    pos+=(vel * player_type.playerSpeedMax() / 0.4);
    AngleDeg body = player.body();
    int dash_cycle;
    int turn_cycle;
    int view_cycle;
    int n_step = player.cycles_to_cut_ball(wm,ball_pos,cycle,false,dash_cycle,turn_cycle,view_cycle,pos,vel,body);
    if( n_step <= cycle)
        return true;
    return false;
