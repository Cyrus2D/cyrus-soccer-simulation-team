	...
	struct Mark {
		...
		, action ( ac )
        {
            ...
            dlog.addText( Logger::SENSOR,
                          __FILE__": set heard mark: "
                          "p1->%d,p2->%d,p3->%d,p4->%d,p5->%d,p6->%d,p7->%d,p8->%d",
                          unums[0],unums[1],unums[2],unums[3],unums[4],unums[5],unums[6],unums[7] );
        }
        int marked_opp(int unum) const{
            if (action.size() > 0 && unum <= action.size()){
                return unums[unum - 1];
            }
            return 0;
        }
        int marked_type(int unum) const{
            if (action.size() > 0 && unum <= action.size()){
                return types[unum - 1];
            }
            return 0;
...
protected:
	...
	std::vector< Mark > M_mark; //!< heard info
	GameTime M_mark_time; //!< info heard time
...
public:
	...
	/*!
      \brief get heard mark info
      \return mark info container
	 */
	const
	std::vector< Mark > & mark() const
	{
		return M_mark;
	}

	/*!
      \brief mark info heard time
      \return time value
	 */
	const
	GameTime & markTime() const
	{
		return M_mark_time;
	}
