...
bool AudioCodec::encodeIntToStr( const std::vector<int>unum,const std::vector<int>type,
                       std::string & to ) const{
    for(int i=0;i<8;i++){
        if(type[i] == 0)
            to+='0';
        else
            to+=markaction2hex(unum[i] + (type[i]-1) * 11);
	}
	return true;
}
...
char
AudioCodec::markaction2hex( const int unum )
{
    if(unum <=9){
        return '0'+unum;
    }else if(unum<=35){
...
*/
int
AudioCodec::hex2markaction( const char hex )
{
    int u = (int)hex;
    if( u>=48 && u <=57 ){
        return u - 48;
