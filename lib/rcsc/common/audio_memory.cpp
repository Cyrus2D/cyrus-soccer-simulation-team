...
*/
void
AudioMemory::setMark( const int sender,
                      const std::vector<int> act,
                      const GameTime & current )
{

    if ( M_mark_time != current )
    {
        M_mark.clear();
    }

    M_mark.push_back(Mark(sender,act) );
    M_mark_time = current;
                      ...
                      bool cross)
{
    ...
    M_pass.push_back( Pass( sender, receiver, pos, prepass, cross ) );;
    M_pass_time = current;
						...
						const int posCount)
{
    ...
    if(sender == unum)
        ...
        tmp.setposcount(posCount + 1);
    M_player.push_back( tmp );
						...
						const int posCount)
{
    dlog.addText( Logger::WORLD,
                  __FILE__": set heard player. sender=%d "
                  "unum=%d pos=(%.2f, %.2f) body=%.1f posCount=%d",
                  sender,
                  unum, pos.x, pos.y, body, posCount );
    if ( M_player_time != current )
    {
        M_player.clear();
    }

    Player tmp = Player( sender, unum, pos, body, stamina );
    if(sender == unum)
        tmp.setposcount(posCount);
    else
        tmp.setposcount(posCount + 1);
